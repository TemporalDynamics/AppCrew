...
async def api_add_candidate(
    ...
    dedup_key = TalentPool.upsert_candidate(signal, run_id="manual_add")
...
async def searches_page(request: Request, workspace: dict = Depends(get_workspace)):
    ...
    try:
        ...
        if db_path.exists():
            ...
            rows = conn.execute(
                """SELECT sr.*,
                    (SELECT COUNT(*) FROM search_matches sm WHERE sm.search_id = sr.search_id) as talents_count,
                    (SELECT COUNT(*) FROM search_matches sm WHERE sm.search_id = sr.search_id AND sm.band = 'priority_1') as shortlist_count,
                    (SELECT COUNT(*) FROM search_matches sm JOIN candidates c ON sm.dedup_key = c.dedup_key WHERE sm.search_id = sr.search_id AND c.status = 'new') as pending_review
        ...
        if db_path.exists():
            ...
            if row:
                ...
                cand_rows = conn.execute(
                    """SELECT sm.dedup_key, sm.score, sm.band, sm.matched_criteria, sm.risks,
                              ci.name, ci.current_role, ci.company, ci.location,
                              c.status, c.first_seen_at
                       FROM search_matches sm
                       JOIN candidates c ON sm.dedup_key = c.dedup_key
                       LEFT JOIN candidate_identities ci ON sm.dedup_key = ci.dedup_key
                       WHERE sm.search_id = ?
...
async def talents_page(request: Request, workspace: dict = Depends(get_workspace)):
    ...
    try:
        ...
        if not db_path.exists():
            ...
            if where_clauses:
                rows = conn.execute(
                    f"""SELECT sm.dedup_key, sm.score, sm.band, sm.matched_criteria, sm.risks, sm.search_id,
                               ci.name, ci.current_role, ci.company, ci.location, ci.skills,
                               c.source, c.status, c.times_surfaced, c.last_seen_at, c.first_seen_at
                        FROM search_matches sm
                        JOIN candidates c ON sm.dedup_key = c.dedup_key
                        LEFT JOIN candidate_identities ci ON sm.dedup_key = ci.dedup_key
                        {where_sql}
...
async def setup_page(workspace: dict = Depends(get_workspace)):
    workspace_id = workspace.get("id", "default")
    path = _criteria_path(workspace_id)
